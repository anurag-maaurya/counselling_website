...
// Assuming RequestContext is defined in ../context/RequestContext
...
export default function MentorLogin() {
  ...
  // 🔐 Default credentials (you can later connect to backend auth)
  ...
  useEffect(() => {
    if (isLoggedIn) {
      // NOTE: Replace with your actual backend endpoint if deploying
  ...
  if (!isLoggedIn) {
    return (
      // Main container background adjusted
      <div className="flex items-center justify-center min-h-screen bg-neutral-900">
        {/* Login Box: bg-neutral-800 for contrast */}
        <div className="bg-neutral-800 p-10 rounded-xl shadow-2xl shadow-black/50 w-96 border border-gray-700">
          ...
          <p className="text-sm text-gray-500 text-center mt-6">
            Default ID: <b className="text-gray-300">mentor123</b> | Password: <b className="text-gray-300">admin@123</b>
          </p>
        </div>
      </div>
    );
          ...
          <table className="w-full border-collapse text-sm text-gray-300">
            <thead>
              <tr className="bg-neutral-900 sticky top-0 shadow-md">
                <th className="border-b border-gray-700 p-3 text-left text-orange-400">Name</th>
                <th className="border-b border-gray-700 p-3 text-left text-orange-400">Mobile</th>
                <th className="border-b border-gray-700 p-3 text-left text-orange-400">Counseling Type</th>
                <th className="border-b border-gray-700 p-3 text-left text-orange-400">Added On</th>
              </tr>
            </thead>
            <tbody>
              {students.length === 0 ? (
                <tr>
                  <td colSpan="4" className="text-center p-4 text-gray-500 bg-neutral-800">
                    No students added yet
                  </td>
                </tr>
              ) : (
                students.map((s) => (
                  <tr key={s._id} className="even:bg-neutral-900 hover:bg-neutral-700 transition duration-150">
                    <td className="border-r border-gray-800 p-3 text-white">{s.name}</td>
                    <td className="border-r border-gray-800 p-3">{s.mobile}</td>
                    <td className="border-r border-gray-800 p-3 text-orange-400">{s.counselingType}</td>
              ...
              >
                <div>
                  <p className="font-medium text-white mb-1">
                    {req.name} <span className="text-orange-500 text-xs ml-2 uppercase">({req.type})</span>
                  </p>
                ...
                <button
                  ...
                  // Resolve button remains bright green for clear actionable contrast
